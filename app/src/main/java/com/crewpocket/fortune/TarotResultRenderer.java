...
final class TarotResultRenderer {
    ...
    void addTarotTopicAnalysisTab() {
        ...
        TextView intro = host.text(
                "先看個性、工作、資源與感情對你代表什麼；數字、牌與週期只放在後面的依據。",
                13, MainActivity.MUTED, false);
        intro.setLineSpacing(host.dp(3), 1f);
        panel.addView(intro);

        LinearLayout personality =
                host.topicCard(panel, "個性與內外", "先看你給人的感覺，和內在真正需要什麼");
        host.addTopicLine(personality, "先說重點",
                host.rendererCopy() != null
                        && !host.rendererCopy().personality.isEmpty()
                        ? FortuneResultTabCopy.compactInterpretation(
                                host.rendererCopy().personality, "")
                        : host.localReportSection("內在 vs 外在"));
        host.addTopicLine(personality, "為什麼這樣說",
        ...
        LinearLayout career =
                host.topicCard(panel, "工作", "先看做事方式與目前職涯節奏");
        host.addTopicLine(career, "先說重點",
                host.rendererCopy() != null
                        && !host.rendererCopy().career.isEmpty()
                        ? FortuneResultTabCopy.compactInterpretation(
                                host.rendererCopy().career, "")
                        : FortuneResultTabCopy.compactInterpretation(
                                "",
                                host.localReportSection("工作與財務")));
        host.addTopicLine(career, "目前階段", currentTarotYearSummary());
        host.addTopicLine(career, "為什麼這樣說",
                "生命道路 " + host.rendererFacts().detailText("lifePathDisplay")
        ...
        LinearLayout wealth =
                host.topicCard(panel, "財運與資源", "先看資源使用與成果節奏，不做投資預測");
        host.addTopicLine(wealth, "先說重點",
                host.rendererCopy() != null
                        && !host.rendererCopy().wealth.isEmpty()
                        ? FortuneResultTabCopy.compactInterpretation(
                                host.rendererCopy().wealth, "")
                        : "這裡看的是資源使用、成果節奏與年度主題，不把牌義當成投資預測。");
        host.addTopicLine(wealth, "目前階段", currentTarotYearSummary());
        host.addTopicLine(wealth, "為什麼這樣說",
                "生命道路 " + host.rendererFacts().detailText("lifePathDisplay")
                        + "　·　四大巔峰 "
                        + host.compactValue(host.rendererFacts().detail("pinnacles"))
                        + "\n目前流年 " + host.rendererFacts().detailText("personalYear")
                        + "「" + host.rendererFacts().detailText("personalYearCardName") + "」");
        host.addAskTeacherAction(
                wealth,
        ...
        LinearLayout relationship =
                host.topicCard(panel, "感情與人際", "先看關係模式，再看牌與挑戰數依據");
        host.addTopicLine(relationship, "先說重點",
                host.rendererCopy() != null
                        && !host.rendererCopy().relationships.isEmpty()
                        ? FortuneResultTabCopy.compactInterpretation(
                                host.rendererCopy().relationships, "")
                        : FortuneResultTabCopy.compactInterpretation(
                                "",
                                host.localReportSection("感情與人際")));
        host.addTopicLine(relationship, "目前階段",
                "目前流年 " + host.rendererFacts().detailText("personalYear")
                        + "「" + host.rendererFacts().detailText("personalYearCardName") + "」"
                        + "　·　個人月 " + host.rendererFacts().detailText("personalMonth"));
        host.addTopicLine(relationship, "為什麼這樣說",
                "外在人格牌 " + host.rendererFacts().detailText("personalityCardNumber")
                        + "　·　內在靈魂牌 " + host.rendererFacts().detailText("soulCardNumber")
                        + "\n四大挑戰 "
                        + host.compactValue(host.rendererFacts().detail("challenges")));
        host.addAskTeacherAction(
                relationship,
                "用語音追問感情",
